at > /mnt/user-data/outputs/pdf_journal_final.blade.php << 'ENDOFFILE'
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        header { position: fixed; top: -80px; left: 0; right: 0; line-height: 1.5; }
        footer { position: fixed; bottom: -40px; left: 0; right: 0; text-align: right; font-size: 10px; }
        .page-number:after { content: "Page " counter(page); }
        .title { font-size: 14px; font-weight: bold; }
        .box   { border: 1px solid black; padding: 5px; }
        table  { width: 100%; border-collapse: collapse; }
        th, td { border: 1px solid black; padding: 3px 5px; }
        th     { background-color: #1a2540; color: #ffffff; text-align: center; font-size: 9px; }
        td     { font-size: 9px; }
        ...
        .signature td { border: none; text-align: center; padding-top: 50px; font-size: 10px; }
    </style>
</head>
<body>
        ...
        @foreach($recap as $r)
            ...
            @if($r->chap_code !== $currentChap)
                ...
                <tr class="{{ str_starts_with($r->chap_code, 'A') ? 'section-recette' : 'section-depense' }}">
                    <td colspan="4">{{ $r->chap_code }} — {{ $r->chap_libelle }}</td>
                </tr>
            @endif
            ...
            <tr>
                ...
                <td class="right">{{ $r->recette ? number_format($r->recette, 2, ',', ' ') : '' }}</td>
                <td class="right">{{ $r->depense ? number_format($r->depense, 2, ',', ' ') : '' }}</td>
            </tr>
        ...
        <td style="width:48%; vertical-align:top; border:none; padding-right:10px;">
            <table>
                <tbody>
                    ...
                    <tr><td>BNI</td>
                        <td class="right">{{ $soldePrecedent->journal_solde_bni ? number_format($soldePrecedent->journal_solde_bni, 2, ',', ' ') : '—' }}</td></tr>
                    <tr><td>BRED</td>
                        <td class="right">{{ $soldePrecedent->journal_solde_bfv ? number_format($soldePrecedent->journal_solde_bfv, 2, ',', ' ') : '—' }}</td></tr>
                    <tr><td>Caisse</td>
                        <td class="right">{{ $soldePrecedent->journal_solde_caisse ? number_format($soldePrecedent->journal_solde_caisse, 2, ',', ' ') : '—' }}</td></tr>
                    <tr class="totaux"><td>Total</td>
                        <td class="right">{{ $soldePrecedent->total ? number_format($soldePrecedent->total, 2, ',', ' ') : '—' }}</td></tr>
                </tbody>
            </table>
        </td>
        <td style="width:48%; vertical-align:top; border:none; padding-left:10px;">
            <table>
                <tbody>
                    ...
                    <tr><td>BNI</td>
                        <td class="right">{{ $soldeCourant->journal_solde_bni ? number_format($soldeCourant->journal_solde_bni, 2, ',', ' ') : '—' }}</td></tr>
                    <tr><td>BRED</td>
                        <td class="right">{{ $soldeCourant->journal_solde_bfv ? number_format($soldeCourant->journal_solde_bfv, 2, ',', ' ') : '—' }}</td></tr>
                    <tr><td>Caisse</td>
                        <td class="right">{{ $soldeCourant->journal_solde_caisse ? number_format($soldeCourant->journal_solde_caisse, 2, ',', ' ') : '—' }}</td></tr>
                    <tr class="totaux"><td>Total</td>
                        <td class="right">{{ $soldeCourant->total ? number_format($soldeCourant->total, 2, ',', ' ') : '—' }}</td></tr>
                </tbody>
            </table>
        </td>
    </tr>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="center">{{ \Carbon\Carbon::parse($d->j_detail_date)->format('d/m/Y') }}</td>
